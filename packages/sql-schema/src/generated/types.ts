...
export interface SqlEngineInfo {
  ...
  name: "sqlserver";
  /**
   * Engine major version as a decimal string, e.g. '12' for Azure SQL Database, or null when the source does not report one.
   */
  version: string | null;
  /**
   * The platform the model targets, e.g. 'SqlAzure' or 'Sql160'. Normalized deliberately: the raw schema-provider string a project declares is a tooling implementation detail, not a fact about the database.
   */
  targetPlatform: string | null;
  /**
   * Collation name, e.g. 'SQL_Latin1_General_CP1_CI_AS'. Null only when the source does not report one.
   */
